...
static const char *strDNSSeed[][2] = {
    ...
    {"prux.info", "prux.info"},
    {"ggvexe4mkwaxdogv.onion"," ggvexe4mkwaxdogv.onion"},
    };

// hidden service seeds
static const char *strMainNetOnionSeed[][1] = {
    {"ggvexe4mkwaxdogv.onion"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. prux is probably already running."), addrBind.ToString().c_str());
        else
...
{
    if (!CreateThread(ThreadTorNet, NULL))
        printf("Error: CreateThread(ThreadTorNet) failed\n");
}
...
void StartNode(void* parg)
{
    ...
    if (!(isfTor == 1) || (fTorEnabled != 1))
        ...
        if (!CreateThread(ThreadOnionSeed, NULL))
            printf("Error: CreateThread(ThreadOnionSeed) failed\n");
